...
const ReviewSection: React.FC<ReviewSectionProps> = ({ productName, rating, ratingBreakdown }) => {
  ...
  return (
    <div className='w-[100%] my-10 flex flex-col m-auto'>
      ...
      <div className='flex my-10 flex-col gap-5 lg:flex-row bg-white shadow-sm rounded-xl border border-gray-200 px-10 py-10'>
        ...
        <div className='flex flex-1 flex-col md:flex-row gap-5'>
          ...
          <div className='flex gap-2 flex-wrap items-center mx-5'>
            <label className='text-6xl font-extrabold text-[rgba(0,0,0,0.7)]'>{rating}</label>
        ...
        <div className='flex items-center mx-5 hover:scale-105 transition-transform duration-300'>
          <button className='text-xl font-semibold text-white bg-blue-600 px-6 py-2 rounded-md lg:w-fit w-full text-nowrap' onClick={()=>setOpenReviewForm(true)}>
            Write a Review
          </button>
        </div>
      </div>
      ...
      {/* users reviews */}
      {review.map(({name,title,review,productReviewImages,rating,date},index)=>(
        <div key={index} className='my-3'>
          <ReviewTemplat name={name} title={title} review={review} productReviewImages={productReviewImages} rating={rating} date={date}/>
        </div>
      ))}
