...
cartRoutes.patch(
  ...
  async (req: Request, res: Response): Promise<any> => {
    ...
    // 1. Basic field check
    if (
      quantity === undefined || // quantity can be 0
      !productId ||
      !customerId
    ) {
    ...
    try {
      ...
      const exists = await userCartDataModel.exists({ productId, customerId });
      if (!exists) {
      ...
      if (quantity === 0) {
        await userCartDataModel.deleteOne({ productId, customerId });
      ...
      const result = await userCartDataModel.updateOne(
...
cartRoutes.delete(
  ...
  async (req: Request, res: Response): Promise<any> => {
    ...
    try {
      ...
      // 3. Delete it
      const result = await userCartDataModel.deleteOne({ productId, customerId });
...
cartRoutes.get("/userCartData", async (req: Request, res: Response): Promise<any> => {
  const { customerId } = req.query;

  try {
    if (customerId) {
      const data = await userCartDataModel.find({ customerId });
    ...
    const allUsers = await userCartDataModel.find();
    return res.status(200).json({
