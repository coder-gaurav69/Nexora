...
const Auth = () => {
  ...
  const { setIsLoggedIn ,setCustomerId} = useContext(GlobalContext) as any;
  ...
  const handleSubmitForm = async () => {
    try {
      ...
      const data = activeTab === "signin" ? signIn : signUp;
      const payload = {...data,type:"member"}
      const response = await axios.post(url, payload, {
        withCredentials: true,
      });
      setCustomerId((response.data as any).customerId)
      console.log((response.data as any).customerId)
      setIsLoggedIn(true);
      navigate("/add");
    } catch (error) {
      console.log(error);
  ...
  return (
    <div className="flex items-center justify-center w-full  ml-auto p-6 z-10 min-h-screen ">
